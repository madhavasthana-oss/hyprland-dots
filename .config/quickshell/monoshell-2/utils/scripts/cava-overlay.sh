...
# Lives under monoshell/utils/scripts; cava config at monoshell/assets/cava/config
set -uo pipefail

CLASS="monoshell-cava"
PIDFILE="${XDG_RUNTIME_DIR:-/tmp}/monoshell-cava.pid"
...
[[ -f "$CAVA_CONF" ]] || CAVA_CONF="${HOME}/.config/quickshell/monoshell/assets/cava/config"
[[ -f "$CAVA_CONF" ]] || CAVA_CONF="${HOME}/Doomslayer-mod/.config/quickshell/monoshell/assets/cava/config"
...
# Place monoshell-cava at full bottom strip via hyprlua (works on this stack)
place_bottom() {
