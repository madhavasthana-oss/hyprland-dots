...
Item {
    ...
    function activateLegacy() {
        root.statusMsg = "ASH PALETTE"
        Globals.toast("Theme", "Activating Ash monochrome palette", "Settings")
    ...
    Process {
        ...
        onExited: (code) => {
            ...
            if (code === 0)
                Globals.toast("Theme", "Text & icons tinted (Ash surfaces kept)", "Settings")
            else
    ...
    Process {
        ...
        onExited: (code) => {
            root.statusMsg = code === 0 ? "ASH PALETTE ACTIVE" : "LEGACY ACTIVATE FAILED"
            if (code === 0)
                Globals.toast("Theme", "Ash monochrome restored", "Settings")
        }
    }
